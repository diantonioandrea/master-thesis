...
\begin{corollary} \label{corollary:embedding}
    ...
    and
    \begin{gather}
        \lvert \partial^{\alpha} u(\Vector{x}) - \partial^{\alpha} u(\Vector{y}) \rvert \leq C \lVert \Vector{x} - \Vector{y} \rVert^{\theta} \lVert u \rVert_{k, p} \text{ for } \mu \text{-a.e. } \Vector{x}, \Vector{y} \in \RealNumbersTo{d}.
    \end{gather}
