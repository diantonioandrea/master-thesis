...
\begin{definition}[$\SpaceLp{1}(\Omega, \SigmaAlgebraA, \omega)$]
    \begin{gather}
        \SpaceLp{1}(\Omega, \SigmaAlgebraA, \omega) = \left\{ f \colon \Omega \rightarrow \RealNumbers \text{ measurable such that } \int_{\Omega} \lvert f(\Vector{x}) \rvert ~ d \omega < \infty \right\},
    \end{gather}
    with:
    \begin{gather}
    \lVert f \rVert_{\SpaceLp{1}(\Omega)} = \lVert f \rVert_{1} = \int_{\Omega} \lvert f(\Vector{x}) \rvert d \omega.
    \end{gather}
\end{definition}
...
\begin{theorem}[Beppo Levi, or monotone convergence]
    ...
    \begin{align}
        ...
        & \sup_{n \in \NaturalNumbers} \int_\Omega f_n(\Vector{x}) ~ d \omega < +\infty,
    \end{align}
...
\begin{definition}[$\SpaceLp{p}(\Omega)$]
    ...
    with:
    \begin{gather}
    \lVert f \rVert_{\SpaceLp{p}(\Omega)} = \lVert f \rVert_{p} = \left( \int_{\Omega} \lvert f(\Vector{x}) \rvert^p ~ d \omega \right)^{1/p}.
    \end{gather}
\end{definition}
...
\begin{theorem}[Riesz representation theorem for $1 < p < +\infty$] \label{theorem:riesz_p}
    ...
    \begin{gather}
        \langle f, \varphi \rangle = \int_{\Omega} f(\Vector{x}) u(\Vector{x}) ~ d \omega,
    \end{gather}
    and:
    \begin{gather}
...
\begin{remark}
    ...
    \begin{gather}
        (u, v)_{\SpaceLp{2}(\Omega)} = \int_{\Omega} u(\Vector{x}) v(\Vector{x}) ~ d \omega,
    \end{gather}
