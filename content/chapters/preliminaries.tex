...
\begin{remark}
    Let $p > d$, then for all $u \in \Wkp{1}{p}(\Rd{d})$ there exists $\tilde{u} \in C(\Rd{d})$ such that $u = \tilde{u}$ for $\mu$-a.e $\Vector{x} \in \Rd{d}$.
\end{remark}
...
\begin{corollary}
    Results in \autoref{corollary:embedding} remain true for $\Omega \subset \Rd{d}$.
\end{corollary}
