...
Under assumptions \cref{equation:assumption_regularity_1} and \cref{equation:assumption_regularity_2}, the exact solution $u$ satisfies what follows for all $\varphi \in \SpaceHk{2}(\Omega, \Omega_h)$ and for $\lambda$-a.e. $t \in I$:
\begin{align}
...
Moreover, the bilinear forms $a_h$, $b_h$, $c_h$, and $J^{\kappa}_h$, as well as $l_h$, introduced in \cref{equation:cdr_ah} to \cref{equation:cdr_lh}, are redefined as follows:
\begin{align}
...
The discontinuous Galërkin discrete problem is to find an approximate solution $u_{h, \tau} \in \SpaceSolutionFull$ to \cref{equation:cdr} with the initial-boundary conditions \cref{equation:cdr_dirichlet}, \cref{equation:cdr_neumann}, and \cref{equation:cdr_initial}, such that:
