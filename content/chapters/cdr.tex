...
\begin{align}
    \lVert \Convection(t, \Vector{x}) \rVert & \leq \BoundC{\Convection} \quad \text{ for } \lambda \otimes \omega \text{-a.e. } t, \Vector{x} \in I \times \Omega, \label{equation:ass_6} \\
    \lvert \Divergence \Convection(t, \Vector{x}) \rvert & \leq \BoundC{\Convection} \quad \text{ for } \lambda \otimes \omega \text{-a.e. } t, \Vector{x} \in I \times \Omega, \label{equation:ass_7} \\
    \lvert \Reaction(t, \Vector{x}) \rvert & \leq \BoundC{\Reaction} \quad \text{ for } \lambda \otimes \omega \text{-a.e. } t, \Vector{x} \in I \times \Omega. \label{equation:ass_8}
\end{align}

Finally, assume that there exists $\BoundC{\Reaction \Convection} > 0$ such that:
\begin{align}
    \Reaction(t, \Vector{x}) - \frac{1}{2} \Divergence \Convection(t, \Vector{x}) & \geq \BoundC{\Reaction \Convection} \quad \text{ for} \lambda \text{-a.e. } t, \Vector{x} \in I \times \Omega. \label{equation:ass_9}
\end{align}

\newpage
\subsection{Weak Solution}

Following \cite{Feistauer2004}, set $V = \left\{ u \in \SpaceHk{1}(\Omega) \text{ such that } u \Restriction_{\partial \Omega^-} = 0 \right\}$.
...
\begin{definition}[$\SpaceHk{k}(\Omega, \Omega_h)$] % [!] Check u, break.
    ...
    \begin{gather}
        \SpaceHk{k}(\Omega, \Omega_h) = \left\{ u \in \SpaceLp{2}(\Omega) \text{ such that } u \Restriction_{K} \in \SpaceHk{k}(K) \text{ for all } K \in \Omega_h \right\}.
    \end{gather}
...
For $u \in \SpaceHk{1}(\Omega, \Omega_h)$, $u \Restriction_{\Gamma_{ij}}$ corresponds to the trace of $u \Restriction_{K_i}$ on $\Gamma_{ij}$ while $u \Restriction_{\Gamma_{ji}}$ corresponds to the trace of $u \Restriction_{K_j}$ on $\Gamma_{ji}$\footnote{$\Gamma_{ji} = \Gamma_{ij}$.}. Moreover:
\begin{align}
    \langle u \rangle_{\Gamma_{ij}} &= \frac{1}{2} \left( u \Restriction_{\Gamma_{ij}} + u \Restriction_{\Gamma_{ji}} \right), \\
    \left[ u \right]_{\Gamma_{ij}} &= u \Restriction_{\Gamma_{ij}} - u \Restriction_{\Gamma_{ji}},
\end{align}
...
\begin{definition}[$\SpaceSolution_h^p$]
    \begin{gather}
        \SpaceSolution_h^p = \left\{ u \in \SpaceLp{2}(\Omega) \text{ such that } u \Restriction_{K} \in \SpacePolynomials{p}(K) \text{ for all } K \in \Omega_h \right\}.
    \end{gather}
\end{definition}
...
\begin{definition}[$\SpaceSolution_{h, n}^p$]
    \begin{gather}
        \SpaceSolution_{h, n}^p = \left\{ u \in \SpaceLp{2}(\Omega) \text{ such that } u \Restriction_{K} \in \SpacePolynomials{p}(K) \text{ for all } K \in \Omega_{h, n} \right\}.
    \end{gather}
\end{definition}
...
\begin{definition}[$\SpaceSolutionFull$]
    ...
    \begin{gather} % [!] To be fixed.
        \SpaceSolutionFull = \left\{ u \in \SpaceLp{2}(I \times \Omega) \text{ such that } u \Restriction_{I_n} = \sum_{i = 0}^q t^i u_i \text{ where } \left\{ u_i \right\}_{i = 0}^q \subset \SpaceSolution_{h, n}^p \text{ for all } n \in \TimeIndices \right\}.
    \end{gather}
\end{definition}
...
\begin{gather}
    \frac{h_K}{\rho_K} \leq \BoundC{\tau} \quad \text{ for all } K \in \Omega_{h, n} \text{ and for all } n \in \TimeIndices. \label{equation:ass_h}
\end{gather}
...
\begin{gather}
    h_{K_i} \leq \BoundDeg \Diameter \Gamma_{ij} \quad \text{ for all } i \in \SpaceTimeIndices, j \in \NeighboursTime{i} \text{ and for all } n \in \TimeIndices. \label{equation:ass_hd}
\end{gather}
...
\begin{lemma}[Multiplicative trace inequality]
    ...
    \begin{gather}
        \lVert u \rVert_{\SpaceLp{2}(\partial K)} \leq \BoundTrace \left( \lVert u \rVert_{\SpaceLp{2}(K)} \lvert u \rvert_{\SpaceHk{1}(K)} + h_K^{-1} \lVert u \rVert_{\SpaceLp{2}(K)}^2 \right),
    \end{gather}
...
\begin{lemma}[Inverse inequality]
    ...
    \begin{gather}
        \lvert u \rvert_{\SpaceHk{1}(K)} \leq \BoundInverse h_K^{-1} \lVert u \rVert_{\SpaceLp{2}(\partial K)},
    \end{gather}
...
\begin{definition}[$\InterpolantPQ$] \label{definition:interpolant_qp}
    ...
    \begin{align}
        ...
        \int_{I_n} \left( \InterpolantPQ u - u, \varphi \right)_0 ~ d \lambda(t) &= 0 \quad \text{ for all } \varphi \in \SpaceSolutionFullMO, \\
...
It follows that:
\begin{gather}
    \left( \IntU \right) \Restriction_{I_n} = \IntIU \quad \text{ for all } n \in \TimeIndices,
\end{gather}
where:
\begin{gather}
...
\begin{lemma} \label{lemma:legendre_interpolant}
    ...
    where:
    \begin{gather}
        \InterSol w(1^-) = \sum_{i \in \NaturalNumbers} w_i \Legendre_i \quad{ with } \left\{ w_i \right\}_{i \in \NaturalNumbers} \subset \SpaceSolution^p_{h, n},
    \end{gather}
    for all $w \in C^0(\left[ -1, 1 \right]; \SpaceLp{2}(\Omega))$ and for all $n \in \TimeIndices$. 
\end{lemma}

Set now $\AppU_n = \InterSol u \circ \RMap$, then:
\begin{gather}
    \eta \Restriction_{I_n} = (\InterpolantPQ u - u) \Restriction_{ I_n} = \IntIU \pm \InterSol u - u \Restriction_{I_n} = \InterpolantPQ \InterSol u \pm \InterSol u - u \Restriction_{I_n},
\end{gather}
...
\begin{align}
    ...
    &= \lVert u - \InterSol u \rVert_{\SpaceLp{2}(I_n; \SpacePlaceholder)} + \sqrt{\frac{\tau_n}{2}} \lVert \AppU_n - \InterpolantPQR \AppU_n \rVert_{\SpaceLp{2}(I_n; \SpacePlaceholder)}.
\end{align}

Moreover, consider the following expansion of $\AppU_n$ with respect to the Legendre polynomials:
\begin{gather}
...
it follows that:
\begin{gather}
    \AppU_n - \InterPoly \AppU_n = \sum_{i = q + 1}^{+\infty} \AppU_{n, i} \Legendre_i.
\end{gather}

Then, \cref{lemma:legendre_interpolant} implies:
\begin{align}
    \AppU_n - \InterpolantPQR \AppU_n &= \sum_{i = q}^{+\infty} \AppU_{n, i} \Legendre_i - \left( \InterSol \AppU_n(1) - \sum_{i = 0}^{q - 1} \AppU_{n, i} \right) \Legendre_q \notag \\
    &= \sum_{i = q}^{+\infty} \AppU_{n, i} \Legendre_i - \left( \sum_{i = q + 1}^{+\infty} \AppU_{n, i} \right) \Legendre_q \notag \\
    &= \AppU_n - \InterPoly \AppU_n - \left( \sum_{i = q + 1}^{+\infty} \AppU_{n, i} \right) \Legendre_q.
\end{align}

\begin{lemma}
    There exists $\BoundQ > 0$ such that:
    \begin{gather}
        \left\lVert \sum_{i = q + 1}^{+\infty} \AppU_{n, i} \right\rVert_{\SpacePlaceholder} \leq \BoundQ \lVert \partial_t \AppU_n \rVert_{\SpaceLp{2}((-1, 1); \SpacePlaceholder)}.
    \end{gather}
\end{lemma}

\begin{lemma}
    \begin{gather}
        \lVert \partial_t \AppU_n \rVert_{\SpaceLp{2}((-1, 1); \SpacePlaceholder)}^2 = \sqrt{\frac{\tau_n}{2}} \lVert \partial_t \left( \InterSol u \right) \rVert_{\SpaceLp{2}(I_n; \SpacePlaceholder)}^2
    \end{gather}
\end{lemma}
...
\begin{lemma}
    \begin{align}
        ...
        &+ \BoundQ \frac{\tau_n}{2} \lVert \partial_t \left( \InterSol u \right) - \partial_t \varphi \rVert_{\SpaceLp{2}(I_n; \SpacePlaceholder)} \quad \text{ for all } \varphi \in \SpaceSolutionFull. \notag
    \end{align}
\end{lemma}

Assume, now, that the solution is sufficiently regular, that is:
\begin{gather}
    u \in \SpaceRegular = \SpaceHk{q + 1}(I; \SpaceHk{1}(\Omega)) \cap C^0(I; \SpaceHk{p + 1}(\Omega)) \quad \text{ for some integers } p, q \geq 1.
\end{gather}

\begin{lemma} % [!] Different time intervals.
    There exist $\BoundInterp, \BoundPoly > 0$ such that:
    \begin{align}
        \lVert w(t) - \InterSol w(t) \rVert_{\SpaceLp{2}(K)} &\leq \BoundInterp h_n^{p + 1} \lvert w(t) \rvert_{\SpaceHk{p + 1}(K)} \quad \text{ for all } K \in \Omega_{h, n} \text{ and } \lambda \text{-a.e. } t \in I_n, \\
        \lvert w(t) - \InterSol w(t) \rvert_{\SpaceHk{1}(K)} &\leq \BoundInterp h_n^{p} \lvert w(t) \rvert_{\SpaceHk{p + 1}(K)} \quad \text{ for all } K \in \Omega_{h, n} \text{ and } \lambda \text{-a.e. } t \in I_n, \\
        \lVert w(t) - \InterSol w(t) \rVert_{\SpaceLp{2}(\Omega)} &\leq \BoundInterp h^{p + 1} \lvert w(t) \rvert_{\SpaceHk{p + 1}(\Omega)} \quad \text{ for } \lambda \text{-a.e. } t \in I, \\
        \lvert w(t) - \InterSol w(t) \rvert_{\SpaceHk{1}(\Omega, \Omega_{h, n})} &\leq \BoundInterp h^{p} \lvert w(t) \rvert_{\SpaceHk{p + 1}(\Omega)} \quad \text{ for } \lambda \text{-a.e. } t \in I, \\
        \lvert w(t) - \InterSol w(t) \rvert_{\SpaceHk{2}(\Omega, \Omega_{h, n})} &\leq \BoundInterp h^{p - 1} \lvert w(t) \rvert_{\SpaceHk{p + 1}(\Omega)} \quad \text{ for } \lambda \text{-a.e. } t \in I, \\
        \lVert w(\Vector{x}) - \InterPolyN w(\Vector{x}) \rVert_{\SpaceLp{2}(I_n)} &\leq \BoundPoly \tau_n^{q + 1} \lvert w(\Vector{x}) \rvert_{\SpaceHk{q + 1}(I_n)} \quad \text{ for } \omega \text{-a.e. } \Vector{x} \in \Omega, \\
        \lvert w(\Vector{x}) - \InterPolyN w(\Vector{x}) \rvert_{\SpaceHk{1}(I_n)} &\leq \BoundPoly \tau_n^{q} \lvert w(\Vector{x}) \rvert_{\SpaceHk{q + 1}(I_n)} \quad \text{ for } \omega \text{-a.e. } \Vector{x} \in \Omega,
    \end{align}
    for all $w \in \SpaceRegular$, and for all $n \in \TimeIndices$.
\end{lemma}
...
\begin{lemma}
    ...
    \begin{gather}
        \frac{1}{\BoundHTVariant} h_K \leq \tau_n \leq \BoundHT h_K \quad \text{ for all } K \in \Omega_{h, n} \text{ and for all } n \in \TimeIndices, \label{equation:ass_ht}
    \end{gather}
    then:
    \begin{align}
...
\begin{theorem}
    ...
    \begin{align}
        \sum_{n \in \TimeIndices} \TimeIntN{ \Norm{\Error}_{E, n} } &\leq C h^{2p} \left( \Seminorm{u}_{\SpaceLp{2}(I; \SpaceHk{p + 1}(\Omega))}^2 + \Seminorm{u}_{C^0(\overline{I}; \SpaceHk{p + 1}(\Omega))}^2 \right) \\
