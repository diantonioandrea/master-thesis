This chapter presents an application of the \acrshort{dgm} developed in \Cref{chapter:dg} to the solution of \acrshort{cdr} problems.
...
\begin{proof}
    ...
    \begin{align*}
        \theta_n^2(\eta; h, \Diffusion) &\leq 4 \left( \left\lVert \eta \right\rVert_{E, n}^2 + \Diffusion h^2 \left\lvert \eta \right\rvert_{\SpaceHk{2}(\Omega, \Omega_{h, n})}^2 \vphantom{\sum_{i \in \SpaceTimeIndices}} \right. \\
        & \left. + \sum_{i \in \SpaceTimeIndices} \left( h_{K_i}^{-2} \left\lVert \eta \right\rVert_{\SpaceLp{2}(K_i)} + \left\lVert \eta_n^- \right\rVert_{\Convection, \partial K_i^- \setminus \partial \Omega} \right) \right),
    \end{align*}
    where:
    \begin{align*}
