...
\begin{definition}[Weak solution]
    ...
    \begin{align}
        ...
        &+ \int_{\Omega} \Reaction u \varphi ~ d \omega(\Vector{x}) = \int_{\Omega} g \varphi ~ d \omega(\Vector{x}) + \int_{\partial \Omega^+} u_N \varphi ~ d \sigma(\Vector{x}),
    \end{align}
    for all $\varphi \in V$.
\end{definition}
...
\begin{align}
    \left( \partial_t u_h(t), \varphi_h \right)_0 + A_h(u_h(t), \varphi_h) &= l_h(\varphi_h) &\text{ for all } \varphi_h \in \SpaceSolution_h^p \text{ and for all } t \in I, \\
    \left( u_h(0), \varphi_h \right)_0 &= \left( u_0, \varphi_h \right)_0 &\text{ for all } \varphi_h \in \SpaceSolution_h^p.
\end{align}
...
\begin{align}
    u_n^+ &= u(t_n^+) = \lim_{t \rightarrow t_n^+} u(t) &\text{ for all } n \in \TimeIndices, \\
    u_n^- &= u(t_n^-) = \lim_{t \rightarrow t_n^-} u(t) &\text{ for all } n \in \TimeIndices, \\
    \left\{ u \right\}_n &= u_n^+ - u_n^- &\text{ for all } n \in \TimeIndices.
\end{align}
...
\begin{align} % [!] Fix alignment and (?) u(0).
    ...
    &= \sum_{n \in \TimeIndices} \int_{I_n} l_{h, n}(\varphi) ~ d \lambda(t) + (u_0, \varphi_0^+ )_0, \label{equation:cdr_discrete}
\end{align}
for all $\varphi \in \SpaceSolutionFull$
...
\begin{align}
    B(u_{h, \tau}, \varphi) &= L(\varphi) &\text{ for all } \varphi \in \SpaceSolutionFull. \label{equation:cdr_discrete_short}
\end{align}
...
where:
\begin{align}
    \lVert u \rVert_{\Convection, \Gamma}^2 &= \int_{\Gamma} \lvert \Convection \cdot \Vector{n} \rvert u^2 ~ d \sigma(\Vector{x}) &\text{ for all } K \in \Omega_{h, n} \text{ where } \Gamma \subset \partial K.
\end{align}
...
\begin{lemma}[Coercivity of $A_{h, n}$]
    ...
    \begin{align}
        A_{h, n}(u, u) &\geq \lVert u \rVert_{E, n}^2 &\text{ for all } u \in \SpaceHk{1}(\Omega, \Omega_{h, n}).
    \end{align}
\end{lemma}
...
\begin{definition}[$\InterpolantPQ$] \label{definition:interpolant_qp}
    ...
    \begin{align}
        ...
        \int_{I_n} \left( \InterpolantPQ u - u, \varphi \right)_0 ~ d \lambda(t) &= 0 &\text{ for all } \varphi \in \SpaceSolutionFullMO, \\
...
\begin{definition}[$\InterpolantPQR$] \label{definition:interpolant_ref_qp}
    ...
    \begin{align}
        ...
        \int_{I_n} \left( \InterpolantPQR u - u, \varphi \right)_0 ~ d \lambda(\zeta) &= 0 &\text{ for all } \varphi \in \SpaceSolutionReferenceMO, \\ 
...
It follows that:
\begin{align}
    \left( \IntU \right) \Restriction_{I_n} = \IntIU &\text{ for all } n \in \TimeIndices,
\end{align}
where:
\begin{align}
...
\begin{align}
    \AppU_n = \sum_{i \in \NaturalNumbers} \AppU_{n, i} \Legendre_i &\text{ for } \left\{ \AppU_{n, i} \right\}_{i \in \NaturalNumbers} \subset \SpaceSolution^p_{h, n}.
\end{align}
Since:
\begin{align}
...
\begin{lemma}
    \begin{align}
        ...
        &+ \BoundQ \frac{\tau_n}{2} \lVert \partial_t \left( \InterSol u \right) - \partial_t \varphi \rVert_{\SpaceLp{2}(I_n; \SpacePlaceholder)} &\text{ for all } \varphi \in \SpaceSolutionFull. \notag
    \end{align}
\end{lemma}
...
\begin{lemma} % [!] Different time intervals, [!] to be fixed.
    ...
    \begin{align}
        \lVert w(t) - \InterSol w(t) \rVert_{\SpaceLp{2}(K)} &\leq \BoundInterp h_n^{p + 1} \lvert w(t) \rvert_{\SpaceHk{p + 1}(K)} &\text{ for all } K \in \Omega_{h, n} \text{ and } \lambda \text{-a.e. } t \in I_n, \\
        \lvert w(t) - \InterSol w(t) \rvert_{\SpaceHk{1}(K)} &\leq \BoundInterp h_n^{p} \lvert w(t) \rvert_{\SpaceHk{p + 1}(K)} &\text{ for all } K \in \Omega_{h, n} \text{ and } \lambda \text{-a.e. } t \in I_n, \\
        \lVert w(t) - \InterSol w(t) \rVert_{\SpaceLp{2}(\Omega)} &\leq \BoundInterp h^{p + 1} \lvert w(t) \rvert_{\SpaceHk{p + 1}(\Omega)} &\text{ for } \lambda \text{-a.e. } t \in I, \\
        \lvert w(t) - \InterSol w(t) \rvert_{\SpaceHk{1}(\Omega, \Omega_{h, n})} &\leq \BoundInterp h^{p} \lvert w(t) \rvert_{\SpaceHk{p + 1}(\Omega)} &\text{ for } \lambda \text{-a.e. } t \in I, \\
        \lvert w(t) - \InterSol w(t) \rvert_{\SpaceHk{2}(\Omega, \Omega_{h, n})} &\leq \BoundInterp h^{p - 1} \lvert w(t) \rvert_{\SpaceHk{p + 1}(\Omega)} &\text{ for } \lambda \text{-a.e. } t \in I, \\
        \lVert w(\Vector{x}) - \InterPolyN w(\Vector{x}) \rVert_{\SpaceLp{2}(I_n)} &\leq \BoundPoly \tau_n^{q + 1} \lvert w(\Vector{x}) \rvert_{\SpaceHk{q + 1}(I_n)} &\text{ for } \omega \text{-a.e. } \Vector{x} \in \Omega, \\
        \lvert w(\Vector{x}) - \InterPolyN w(\Vector{x}) \rvert_{\SpaceHk{1}(I_n)} &\leq \BoundPoly \tau_n^{q} \lvert w(\Vector{x}) \rvert_{\SpaceHk{q + 1}(I_n)} &\text{ for } \omega \text{-a.e. } \Vector{x} \in \Omega,
    \end{align}
