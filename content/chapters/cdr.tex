...
Furthermore, set:
\begin{align}
    h_n &= \max_{K \in \Omega_{h, n}} h_K \quad \text{ for all } n \in \TimeIndices, \\
    h &= \max_{n \in \TimeIndices} h_n, \\
    \tau &= \max_{n \in \TimeIndices} \lambda(I_n) = \max_{n \in \TimeIndices} t_n - t_{n - 1}.
\end{align}
...
Moreover, the bilinear forms $a_h$, $b_h$, $c_h$, and $J^{\kappa}_h$, as well as $l_h$, introduced in \eqref{equation:cdr_ah} to \eqref{equation:cdr_lh}, are redefined as follows:
\begin{align}
    ...
    c_{h, n} \left( u, \varphi \right) &= \Reaction \left( u, \varphi \right)_{\SpaceLp{2}(\Omega)}, \label{equation:cdr_chn} \\
    ...
    l_{h, n} \left( \varphi \right)  &= \left( g, \varphi \right)_{\SpaceLp{2}(\Omega)} + \sum_{i \in \SpaceTimeIndices} \int_{\partial K_i^+ \cap \partial \Omega} u_N \varphi ~ d \sigma(\Vector{x}) \label{equation:cdr_lhn} \\
...
\begin{definition}[$\SpaceSolution_{h, \tau}^{p, q}$]
