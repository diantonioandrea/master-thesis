...
\end{align}
and $\Vector{n}_{ij}$ denotes the unit outer normal with respect to $\partial K_i$ on $\Gamma_{ij}$. Note that, geometrically, $\Gamma_{ji} = \Gamma_{ij}$.
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &- \sum_{i \in \SpaceTimeIndices} \int_{\partial K_i^- \cap \partial \Omega} \left( \Convection \cdot \Vector{n}_i \right) w^2 ~ d \sigma(\Vector{x}) \\
        &- \sum_{i \in \SpaceTimeIndices} \int_{\partial K_i^- \setminus \partial \Omega} \left( \Convection \cdot \Vector{n}_i \right) \left( w - w^- \right) w ~ d \sigma(\Vector{x}),
    \end{align*}
...
\begin{definition}[$\InterpolantPQ$] \label{definition:interpolant_qp}
    ...
    \end{align}
    for all $n \in \TimeIndices$, where $\InterSol$ denotes the $\SpaceLp{2}$ projection onto $\SpaceSolution^p_{h, n}$.
\end{definition}
...
\end{align}
and note that $\xi \in \SpaceSolutionFull$ and $\eta$ denotes the interpolation error, then $\Error = \xi + \eta$, and \cref{equation:error_1} can be rewritten as:
\begin{align}
...
\end{align}
where $\SpacePlaceholder$ denotes a space that is yet to be specified.
