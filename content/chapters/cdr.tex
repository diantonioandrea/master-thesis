...
\begin{gather}
    h_{K_i} \leq \BoundDeg \Diameter \Gamma_{ij} \quad \text{ for all } i \in \SpaceTimeIndices, j \in \NeighboursTime{i} \text{ and for all } n \in \TimeIndices.
\end{gather}
...
\begin{definition}[$\IntU$] \label{definition:interpolant_qp}
    Let $\InterpolantPQ$ such that:
    \begin{align}
        \IntU &\in \SpaceSolutionFull, \\
        \int_{I_n} \left( \IntU - u, \varphi \right)_0 ~ d \lambda(t) &= 0 \quad \text{ for all } \varphi \in \SpaceSolutionFullMO, \\
        \IntU(t_n^-) &= \ProjectionOnto{\SpaceSolution^p_{h, n}} u(t_n^-),
    \end{align}
...
Set now $\AppU_n = \ProjectionOnto{\SpaceSolution^p_{h, n}} u \circ \RMap$, then:
\begin{gather}
...
\begin{lemma}
    \begin{align}
        ...
        &+ \BoundQ \frac{\tau_n}{2} \lVert \partial_t \left( \ProjectionOnto{\SpaceSolution^p_{h, n}} u \right) - \partial_t \varphi \rVert_{\SpaceLp{2}(I_n; \SpacePlaceholder)} \quad \text{ for all } \varphi \in \SpaceSolutionFull. \notag
    \end{align}
\end{lemma}
...
\begin{lemma} % [!] Different time intervals.
    ...
    \begin{align}
        \lVert w(t) - \ProjectionOnto{\SpaceSolution^p_{h, n}} w(t) \rVert_{\SpaceLp{2}(K)} &\leq \BoundInterp h_n^{p + 1} \lvert w(t) \rvert_{\SpaceHk{p + 1}(K)} \quad \text{ for all } K \in \Omega_{h, n} \text{ and } \lambda \text{-a.e. } t \in I_n, \\
        \lvert w(t) - \ProjectionOnto{\SpaceSolution^p_{h, n}} w(t) \rvert_{\SpaceHk{1}(K)} &\leq \BoundInterp h_n^{p} \lvert w(t) \rvert_{\SpaceHk{p + 1}(K)} \quad \text{ for all } K \in \Omega_{h, n} \text{ and } \lambda \text{-a.e. } t \in I_n, \\
        \lVert w(t) - \ProjectionOnto{\SpaceSolution^p_{h, n}} w(t) \rVert_{\SpaceLp{2}(\Omega)} &\leq \BoundInterp h^{p + 1} \lvert w(t) \rvert_{\SpaceHk{p + 1}(\Omega)} \quad \text{ for } \lambda \text{-a.e. } t \in I, \\
        \lvert w(t) - \ProjectionOnto{\SpaceSolution^p_{h, n}} w(t) \rvert_{\SpaceHk{1}(\Omega, \Omega_{h, n})} &\leq \BoundInterp h^{p} \lvert w(t) \rvert_{\SpaceHk{p + 1}(\Omega)} \quad \text{ for } \lambda \text{-a.e. } t \in I, \\
        \lvert w(t) - \ProjectionOnto{\SpaceSolution^p_{h, n}} w(t) \rvert_{\SpaceHk{2}(\Omega, \Omega_{h, n})} &\leq \BoundInterp h^{p - 1} \lvert w(t) \rvert_{\SpaceHk{p + 1}(\Omega)} \quad \text{ for } \lambda \text{-a.e. } t \in I, \\
        \lVert w(\Vector{x}) - \ProjectionOnto{\SpacePolynomials{q}(I_n)} w(\Vector{x}) \rVert_{\SpaceLp{2}(I_n)} &\leq \BoundPoly \tau_n^{q + 1} \lvert w(\Vector{x}) \rvert_{\SpaceHk{q + 1}(I_n)} \quad \text{ for } \omega \text{-a.e. } \Vector{x} \in \Omega, \\
        \lvert w(\Vector{x}) - \ProjectionOnto{\SpacePolynomials{q}(I_n)} w(\Vector{x}) \rvert_{\SpaceHk{1}(I_n)} &\leq \BoundPoly \tau_n^{q} \lvert w(\Vector{x}) \rvert_{\SpaceHk{q + 1}(I_n)} \quad \text{ for } \omega \text{-a.e. } \Vector{x} \in \Omega,
    \end{align}
...
\begin{lemma}
    ...
    \begin{align}
        \lVert \ProjectionOnto{\SpaceSolution^p_{h, n}} w \rVert_{\SpaceLp{2}(K)} &\leq \lVert w \rVert_{\SpaceLp{2}(K)}, \\
        \lvert \ProjectionOnto{\SpaceSolution^p_{h, n}} w \rvert_{\SpaceHk{1}(K)} &\leq \BoundH \lvert w \rvert_{\SpaceHk{1}(K)},
    \end{align}
...
\begin{lemma}
    ...
    \begin{gather}
        \frac{1}{\BoundHTVariant} h_K \leq \tau_n \leq \BoundHT h_K \quad \text{ for all } K \in \Omega_{h, n} \text{ and for all } n \in \TimeIndices,
    \end{gather}
    then:
    \begin{align}
...
\begin{theorem}
    ...
    \begin{align}
        ...
        &+ C \tau^{2q} \left( \Seminorm{u}_{\SpaceHk{q + 1}(I; \SpaceLp{2}(\Omega))}^2 + \Seminorm{u}_{\SpaceHk{q + 1}(I_n; \SpaceHk{1}(\Omega))}^2 \right). \notag
    \end{align}
\end{theorem}
This estimated holds true even for $\Diffusion = 0$.