...
\begin{definition}[A] \label{definition:A} % [!] Check instances of C_a
    Let $A \colon I \rightarrow \mathcal{L}(V;V^*)$ be an operator such that the map that associates $\langle A(t)(u), v \rangle_{V^*, V}$ to $t \in I$ is measurable for all $u, v \in V$. Moreover there exist $C_a, \alpha > 0$ such that:
    \begin{align} \label{equation:a}
        & \lVert A(t)(u) \rVert_{V^*} \leq C_a \lVert u \rVert_V \text{ for all } u \in V \text{ and for } \lambda \text{-a.e. } t \in I, \\
...
\begin{lemma}
    ...
    \begin{gather}
        \lVert A(u) \rVert_{\SpaceLp{p}(I; V^*)} \leq C_a \lVert u \rVert_{\SpaceLp{p}(I; V)}.
    \end{gather}
\end{lemma}
...
Following \cite[p. 186]{Ern2021}, recall that $\alpha$ is the coercivity constant of the bilinear form $a$ and equip $\SpaceX_{h \tau}$ with the following norm such that for all $u_{h \tau} \in \SpaceX_{h \tau}$:
\begin{gather}
    \lVert u_{h \tau} \rVert_{\SpaceX_{h \tau}}^2 = \lVert u_{h \tau} \rVert_{\SpaceLp{2}(I; V)}^2 + \frac{1}{2 \alpha} \left( \lVert u_{h \tau}(0) \rVert_W^2 + \lVert u_{h \tau}(T) \rVert_W^2 + \sum_{n \in \TimeIndices} \lVert \llbracket u_{h \tau} \rrbracket_{n - 1} \rVert_W^2 \right).
\end{gather}
...
\begin{lemma}[Coercivity]
    ...
    \begin{gather}
        b_{\tau}(u_{h \tau}, u_{h \tau}) \geq \alpha \lVert u_{h \tau} \rVert_{\SpaceX_{h \tau}}^2 \text{ for all } u_{h \tau} \in \SpaceX_{h \tau}.
    \end{gather}
\end{lemma}
...
\begin{theorem}[$\SpaceLp{2}(I; W)$-estimate]
    Let $u \in \SpaceX$ be the solution of \eqref{equation:model} and $u_{h \tau} \in \SpaceX_{h \tau}$ be the solution of \eqref{equation:weak_ht}. Assume that $u \in \SpaceHk{q + 2}(I; V^*) \cap \SpaceWkp{q + 1}{\infty}(I; V)$, then there exists $C \geq 0$ such that for all $h \in \hIndices$ and $T > 0$:
    \begin{align}
        \lVert u - u_{h \tau} \rVert_{\SpaceX_{h \tau}} \leq & \frac{\sqrt{2}}{\sqrt{\alpha}} \lVert u_0 - \InterpolantH(u_0) \rVert_W + \frac{C}{\alpha} \tau^{q + 1} \lvert u \rvert_{\SpaceHk{q + 2}(I; V^*)} \\
        & + \frac{C C_a}{\alpha} \sqrt{T} \tau^{q + 1} \lvert u \rvert_{\SpaceWkp{q + 1}{\infty}(I; V)} + \frac{C}{\alpha} \lVert u_t - \InterpolantH(u_t) \rVert_{\SpaceLp{2}(I; V^*)} \\
        & + \frac{C C_a}{\alpha} C_* \lVert u - \InterpolantH(u) \rVert_{\SpaceLp{\infty}(I; V)}.
    \end{align}
\end{theorem}