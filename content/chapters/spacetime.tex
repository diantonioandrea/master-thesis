...
\begin{theorem}[Time trace and integration by parts]
    ...
    \begin{align}
        \int_I \langle \partial_t u(t), v(t) \rangle_{V^*, V} ~ d \lambda(t) = &- \int_I \langle \partial_t v(t), u(t) \rangle_{V^*, V} ~ d \lambda(t) + \left( u(T), v(T) \right)_W \\
        &- \left( u(0), v(0) \right)_W. \notag
    \end{align}
\end{theorem}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        2 \left( u_{h \tau}(t_{n - 1}^+) - u_{h \tau}(t_{n - 1}) \right) u_{h \tau}(t_{n - 1}^+) &= u_{h \tau}^2(t_{n - 1}^+) - u_{h \tau}^2(t_{n - 1}) \\ 
        &+ \left( u_{h \tau}(t_{n - 1}^+) - u_{h \tau}(t_{n - 1}) \right)^2,
    \end{align*}
