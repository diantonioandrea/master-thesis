This chapter presents the theory behind the \acrlong{dgm}.

\Cref{sec:bochner} introduces the theory of Bochner spaces, which, together with \Cref{chapter:preliminaries}, completes the mathematical foundations of the \acrshort{dgm}.

The goal of \Cref{sec:model_problem,section:dg_discretization} is to develop the \acrshort{dgm} around a model parabolic problem, for which \Cref{chapter:cdr} provides a specific formulation. \Cref{subsection:introduction,subsection:weak_formulation} introduce the problem, while \Cref{subsection:well_posedeness} establishes its well-posedness. \Cref{sec:space_discretization,subsection:time_dg} describe the space and time discretizations of the problem, and \Cref{subsection:method_formulation} formulates the \acrshort{dgm}. Finally, \Cref{subsection:method_analysis} provides the stability and error analysis of the method.

Only the relevant proofs are included, while the remaining ones can be found in \citetitle{Ern2021}.
...
\begin{lemma} % [!] Ugly \left and \right with \left\lVert and \right\rVert.
    \begin{align}
...
\begin{lemma}
    Let $u\colon I \rightarrow V$ be a Bochner integrable function and $\left\{ u_n \right\}_{n \in \NaturalNumbers}$ as in \Cref{definition:bochner_integrable}, then $\left\{ \int_I u_n(t) ~ d \lambda(t) \right\}_{n \in \NaturalNumbers}$ converges in $V$ with respect to $\left\lVert \cdot \right\rVert_V$.
\end{lemma}
...
\begin{definition}[$\SpaceXpq{p}{q}(I; V, W)$]
    ...
    Moreover, $\SpaceXpq{}{q}(I; V, W)$ is equipped with the norm $\left\lVert \cdot \right\rVert_{\SpaceXpq{}{q}(I; V, W)}$, such that for all \\ $u \in \SpaceXpq{P}{q}(I; V, W)$:
    \begin{align}
        \left\lVert u \right\rVert_{\SpaceXpq{}{q}(I; V, W)} = \left\lVert u \right\rVert_{\SpaceLp{p}(I; V)} + \iota_{V, W}^{-1}T^{1 + \frac{1}{p} - \frac{1}{q}} \left\lVert \partial_t u \right\rVert_{\SpaceLp{q}(I; W)},
    \end{align}
    where:
    \begin{align}
...
\begin{theorem}[$\SpaceLp{2}(I; V)$-estimate] \label{theorem:estimate_h}
    ...
    \begin{align}
        \left\lVert u - u_{h \tau} \right\rVert_{\SpaceLp{2}(I; V)} \leq \left( 1 + \CoCoR{a} \right) \left\lVert \eta \right\rVert_{\SpaceLp{2}(I; V)} + \frac{1}{\CoerC{a}} \left\lVert \partial_t \eta \right\rVert_{\SpaceLp{2}(I; V^*)} + \frac{1}{\sqrt{\CoerC{a}}} \left\lVert \eta(0) \right\rVert_W,
    \end{align}
    where:
    \begin{align}
